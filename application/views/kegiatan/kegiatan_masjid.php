<!doctype html>

<html lang="en">

  <head>

    <meta charset="utf-8">

    <meta name="viewport" content="width=device-width, initial-scale=1">

    <title>DKM MASJID AL FURQON</title>

    <!-- add icon link -->
    <link rel = "icon" href = "<?= base_url() ?>assets/img/kegiatanmasjid/Logo Al-Furqon-01.png" type = "image/x-icon">

    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.2.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-rbsA2VBKQhggwzxH7pPCaAqO46MgnOM80zW1RWuH61DGLwZJEdK2Kadq2F9CUG65" crossorigin="anonymous">

    <link
    rel="stylesheet"
    href="https://cdn.jsdelivr.net/npm/swiper@9/swiper-bundle.min.css"
    />



    <!-- Template Main CSS File -->

    <!-- <link href="<?= base_url() ?>assets/css/style.css" rel="stylesheet"> -->

    <link href="<?= base_url() ?>assets/css/mycss.css" rel="stylesheet">



    <!-- Google Fonts -->

    <link href="https://fonts.googleapis.com/css?family=Open+Sans:300,300i,400,400i,600,600i,700,700i|Raleway:300,300i,400,400i,500,500i,600,600i,700,700i|Poppins:300,300i,400,400i,500,500i,600,600i,700,700i" rel="stylesheet">



  </head>

  <body>

    <div id="carouselExampleSlidesOnly" class="carousel slide" data-bs-ride="carousel">

        <div class="carousel-inner">

            <div class="carousel-item active" data-bs-interval="3000">

                <div class="background">

                    <img src="<?= base_url() ?>assets/img/kegiatanmasjid/alfurqon.jpg" class="d-block w-100" alt="...">

                </div>

                <div class="col-12 pembuka position-absolute top-50 start-50 translate-middle">
                    <div class="judulpembuka text-center">
                        <img src="<?= base_url() ?>assets/img/kegiatanmasjid/rounded_logo.png" class="mb-2" alt="" width="100">
                        <h1>PROGRAM KEGIATAN PEKANAN <br/>MASJID AL FURQON</h1>
                    </div>
                </div>

            </div>

            <div class="carousel-item" data-bs-interval="10000">

                <div class="background">

                    <img src="<?= base_url() ?>assets/img/bg_ramadhan2.jpg" class="d-block w-100" alt="...">

                </div>

                <div class="col-6 kotakthumb position-absolute top-50 end-0 translate-middle-y">
                    <img src="<?= base_url() ?>assets/img/Ustad Ali Shadiqin.png" class=" thumbnail rounded float-end me-auto" alt="...">  
                </div>

                <div class="col-6 d-flex judulkegiatan judulkegiatan1 position-absolute top-50 start-0 translate-middle">

                    <div class="kegiatan1">

                        <h1>SENIN</h1>

                        <h3>Kajian bada subuh s.d Syuruq <br>Tadabur Al Qur'an</h3>

                        <p class="lh-1">Ustd Ali Shodiqin, Lc.</p>

                    </div>

                </div>

            </div>

            <div class="carousel-item" data-bs-interval="10000">

                <div class="background">

                    <img src="<?= base_url() ?>assets/img/bg_ramadhan.jpg" class="d-block w-100" alt="...">

                </div>

                <div class="col-6 kotakthumb position-absolute top-50 end-0 translate-middle-y">
                    <swiper-container class="mySwiper ms-5" navigation="false" space-between="30"
                        centered-slides="true" autoplay-delay="2500" autoplay-disable-on-interaction="false">
                        <swiper-slide>
                            <img src="<?= base_url() ?>assets/img/Ustad Ade Wahidin.png" class=" thumbnail rounded " alt="...">
                        </swiper-slide>
                        <swiper-slide>
                            <img src="<?= base_url() ?>assets/img/Ustad Ali Shadiqin.png" class=" thumbnail rounded " alt="...">
                        </swiper-slide>
                    </swiper-container>
                </div>

                <div class="col-6 judulkegiatan position-absolute top-0 start-0">

                    <div class="selasa1">

                        <h1>SELASA</h1>

                        <h3>Kajian Ba'da Subuh s.d Syuruq <br>Tafsir Al Qur'an</h3>

                        <p class="lh-1">Ustd. Dr. Ade Wahidin, Lc, MA</p>

                    </div>

                    <div class="selasa1">

                        <h3>Tahsin Tilawah Dewasa Akhwat</h3>

                        <p class="lh-1">Pagi Jam 08.00 - 10.00</p>

                        <div class="ustazah">

                            <p class="lh-1">Ustzh. Zita</p>

                        </div>

                    </div>

                    <div class="selasa1">

                        <h3>Diskusi Sejarah Islam</h3>

                        <p class="lh-1">Bada Isya - 21.00</p>

                        <div class="ustazah">

                            <p class="lh-1">Ustd Ali Shodiqin, Lc</p>

                        </div>

                    </div>

                </div>

            </div>

            <div class="carousel-item" data-bs-interval="10000">

                <div class="background">

                    <img src="<?= base_url() ?>assets/img/bg_ramadhan2.jpg" class="d-block w-100" alt="...">

                </div>

                <div class="col-6 kotakthumb position-absolute top-50 end-0 translate-middle-y">

                    <swiper-container class="mySwiper ms-5" navigation="false" space-between="30"
                        centered-slides="true" autoplay-delay="2500" autoplay-disable-on-interaction="false">
                        <swiper-slide>
                            <img src="<?= base_url() ?>assets/img/Ustad Ali Shadiqin.png" class=" thumbnail rounded " alt="...">
                        </swiper-slide>
                        <swiper-slide>
                            <img src="<?= base_url() ?>assets/img/Ustad Ali Shadiqin.png" class=" thumbnail rounded " alt="...">
                        </swiper-slide>
                    </swiper-container>

                </div>

                <div class="col-6 judulkegiatan position-absolute top-0 start-0">

                    <div class="kegiatan1">

                        <h1>RABU</h1>

                        <h3>Kajian bada subuh s.d Syuruq <br>Tadabur Al Qur'an</h3>

                        <p class="lh-1">Ustd Ali Shodiqin, Lc</p>

                    </div>

                    <div class="kegiatan2">

                        <h3>Kajian Pagi Khusus Ummahatt <br>Tadabbur Surah An-Nur</h3>

                        <p class="lh-1">Pagi Jam 08.00 - 10.00</p>

                        <div class="ustazah">

                            <p class="lh-1">Ustd Ali Shodiqin, Lc</p>

                        </div>

                    </div>

                </div>

            </div>

            <div class="carousel-item " data-bs-interval="10000">

                <div class="background">

                    <img src="<?= base_url() ?>assets/img/bg_ramadhan.jpg" class="d-block w-100" alt="...">

                </div>

                <div class="col-4 kotakthumb position-absolute top-50 end-0 translate-middle-y">

                    <swiper-container class="mySwiper ms-5" navigation="false" space-between="30"
                        centered-slides="true" autoplay-delay="2500" autoplay-disable-on-interaction="false">
                        <swiper-slide>
                            <img src="<?= base_url() ?>assets/img/Ustad Ali Shadiqin.png" class=" thumbnail rounded " alt="...">
                        </swiper-slide>
                        <swiper-slide>
                            <img src="<?= base_url() ?>assets/img/Ustad Ali Shadiqin.png" class=" thumbnail rounded " alt="...">
                        </swiper-slide>
                    </swiper-container>

                </div>

                <div class="col-8 judulkegiatan position-absolute top-0 start-0">

                    <div class="kegiatan1">

                        <h1>KAMIS</h1>

                        <h3>Kajian bada subuh s.d Syuruq<br>Kajian Mu’amalah Kitab Al-Mu’amalah Al-Mashrafiyyah</h3>

                        <p class="lh-1">Ustd. Dr. Ade Wahidin, Lc, MA</p>

                    </div>

                    <div class="kegiatan2">

                        <h3>Diskusi Fiqih Islam</h3>

                        <p class="lh-1">Bada Isya - 21.00</p>

                        <div class="ustazah">

                            <p class="lh-1">Ustd Ali Shodiqin, Lc.</p>

                        </div>

                    </div>

                </div>

            </div>

            <div class="carousel-item " data-bs-interval="10000">

                <div class="background">

                    <img src="<?= base_url() ?>assets/img/bg_ramadhan2.jpg" class="d-block w-100" alt="...">

                </div>

                <div class="col-4 kotakthumb position-absolute top-50 end-0 translate-middle-y">

                    <img src="<?= base_url() ?>assets/img/Ustad Ali Shadiqin.png" class=" thumbnail rounded float-end me-auto" alt="...">

                </div>

                <div class="col-8 judulkegiatan position-absolute top-0 start-0">

                    <div class="kegiatan1">

                        <h1>JUMAT</h1>

                        <h3>Kajian bada subuh s.d Syuruq<br>Tadabur Al Qur'an</h3>

                        <p class="lh-1">Ustd Ali Shodiqin, Lc.</p>

                    </div>

                    <div class="kegiatan2">

                        <h3>Bela Diri Kungfu Muslim</h3>

                        <p class="lh-1">Bada Ashar  - 17.30</p>

                        <div class="ustazah">

                            <p class="lh-1">TAKUMU</p>

                        </div>

                    </div>

                </div>

            </div>

            <div class="carousel-item " data-bs-interval="10000">

                <div class="row position-relative">

                    <div class="background">

                        <img src="<?= base_url() ?>assets/img/bg_ramadhan.jpg" class="d-block w-100" alt="...">

                    </div>

                    <div class="col-4 judulkegiatan mt-3 position-absolute top-0 start-0">

                        <div class="ahad1">

                            <h3>Kajian bada subuh s.d Syuruq<br>Tadabur Al Qur'an</h3>

                            <p class="lh-1">Ustd Ali Shodiqin, Lc.</p>

                        </div>

                        <div class="ahad2">

                            <h3>Bela Diri Kungfu Muslim</h3>

                            <p class="lh-1">Bada Ashar  - 17.30</p>

                            <div class="ustazah">

                                <p class="lh-1">TAKUMU</p>

                            </div>

                        </div>

                    </div>

                    <div class="col-4 mt-3 position-absolute top-0 start-50 translate-middle-x">

                        <h1 class="text-center lh-lg" style="color:white; font-size:45pt;">SABTU</h1>
                        <swiper-container class="mySwiper " navigation="false" space-between="30"
                        centered-slides="true" autoplay-delay="2500" autoplay-disable-on-interaction="false">
                            <swiper-slide>
                                <img src="<?= base_url() ?>assets/img/Ustad Herman.png" class=" thumbnail rounded " alt="...">
                            </swiper-slide>
                            <swiper-slide>
                                <img src="<?= base_url() ?>assets/img/Ustad Ali Shadiqin.png" class=" thumbnail rounded " alt="...">
                            </swiper-slide>
                        </swiper-container>

                    </div>

                    <div class="col-4 judulkegiatan mt-3 position-absolute top-0 end-0">

                        <div class="ahad1">

                            <h3>Kajian bada subuh s.d Syuruq<br>Tadabur Al Qur'an</h3>

                            <p class="lh-1">Ustd Ali Shodiqin, Lc.</p>

                        </div>

                        <div class="ahad2">

                            <h3>Bela Diri Kungfu Muslim</h3>

                            <p class="lh-1">Bada Ashar  - 17.30</p>

                            <div class="ustazah">

                                <p class="lh-1">TAKUMU</p>

                            </div>

                        </div>

                    </div>

                </div>

            </div>

            <div class="carousel-item" data-bs-interval="10000">

                <div class="background">

                    <img src="<?= base_url() ?>assets/img/bg_ramadhan2.jpg" class="d-block w-100" alt="...">

                </div>

                <div class="col-6 kotakthumb position-absolute top-50 end-0 translate-middle-y">

                    <swiper-container class="mySwiper ms-5" navigation="false" space-between="30"
                        centered-slides="true" autoplay-delay="2500" autoplay-disable-on-interaction="false">
                            <swiper-slide>
                                <img src="<?= base_url() ?>assets/img/Ustad Herman.png" class=" thumbnail rounded " alt="...">
                            </swiper-slide>
                            <swiper-slide>
                                <img src="<?= base_url() ?>assets/img/Ustad Ali Shadiqin.png" class=" thumbnail rounded " alt="...">
                            </swiper-slide>
                    </swiper-container>

                </div>

                <div class="col-6 judulkegiatan position-absolute top-0 start-0">

                    <div class="selasa1">

                        <h1>AHAD</h1>

                        <h3>Kajian Ba'da Subuh s.d Syuruq <br>Asmaul Husna</h3>

                        <p class="lh-1">Ustd. Ali Shodiqin</p>

                    </div>

                    <div class="selasa1">

                        <h3>Latihan Bela Diri Thifan</h3>

                        <p class="lh-1">Syuruq 06.15 - 09.00</p>

                        <div class="ustazah">

                            <p class="lh-1">Ustd Eko</p>

                        </div>

                    </div>

                    <div class="selasa1">

                        <h3>Mudah Memahami Bahasa Arab</h3>

                        <p class="lh-1">Bada Isya - 21.00</p>

                        <div class="ustazah">

                            <p class="lh-1">Ustd Ali Shodiqin, Lc</p>

                        </div>

                    </div>

                </div>

            </div> 

            <div class="carousel-item" data-bs-interval="10000">

                 <div class="background">

                    <img src="<?= base_url() ?>assets/img/bg_ramadhan.jpg" class="d-block w-100" alt="...">

                    </div>

                    <div class="col-6 kotakthumb position-absolute top-50 end-0 translate-middle-y">

                        <swiper-container class="mySwiper ms-5" navigation="false" space-between="30"
                            centered-slides="true" autoplay-delay="2500" autoplay-disable-on-interaction="false">
                                <swiper-slide>
                                    <img src="<?= base_url() ?>assets/img/kegiatanmasjid/santunan.jpg" class=" thumbnail rounded float-end ms-5">
                                </swiper-slide>
                                <swiper-slide>
                                    <img src="<?= base_url() ?>assets/img/kegiatanmasjid/santunan.jpg" class=" thumbnail rounded float-end ms-5">
                                </swiper-slide>
                        </swiper-container>

                    </div>

                    <div class="col-6 judulkegiatan position-absolute top-0 start-0">

                    <div class="kegiatan1">

                        <h3>KEGIATAN ASMAF SANTUNAN ANAK YATIM</h3>

                        <h5 style="color: #fff;">Santunan kepada 130 Anak Yatim</h5>

                    </div>

                    <div class="kegiatanrutin2">

                        <p>Bank Syariah Indonesia atas nama Masjid Alfurqon</p>

                        <div class="ustazah">

                            <p>No Rekening : 2021202635</p>

                        </div>

                    </div>

                    <div class="qris">

                        <img src="<?= base_url() ?>assets/img/qris/Rek_Asmaf.jpg" class=" thumbnail rounded " style="width:200px !important; height: 200px !important;">

                    </div>

                </div>

            </div>

            <div class="carousel-item" data-bs-interval="10000">

                 <div class="background">

                    <img src="<?= base_url() ?>assets/img/bg_ramadhan2.jpg" class="d-block w-100" alt="...">

                    </div>

                    <div class="col-6 kotakthumb position-absolute top-50 end-0 translate-middle-y">

                        <swiper-container class="mySwiper ms-5" navigation="false" space-between="30"
                            centered-slides="true" autoplay-delay="2500" autoplay-disable-on-interaction="false">
                                <swiper-slide>
                                <img src="<?= base_url() ?>assets/img/kegiatanmasjid/wakaf_parkir.jpg" class=" thumbnail rounded float-end ms-5">
                                </swiper-slide>
                                <swiper-slide>
                                    <img src="<?= base_url() ?>assets/img/kegiatanmasjid/santunan.jpg" class=" thumbnail rounded float-end ms-5">
                                </swiper-slide>
                        </swiper-container>

                    </div>

                    <div class="col-6 judulkegiatan position-absolute top-0 start-0">

                    <div class="kegiatan1">

                        <h3>WAKAF PEMBANGUNAN AREA PARKIR DAN KELAS</h3>

                        <h5 style="color: #fff;">Dana yang dibutuhkan untuk pembangunan area parkir motor dan kelas sebesar Rp 815 juta.

                        </h5>

                    </div>

                    <div class="kegiatanrutin2">

                        <p>Bank Syariah Indonesia atas nama Masjid Alfurqon</p>

                        <div class="ustazah">

                            <p>No Rekening : 2021202646</p>

                        </div>

                    </div>

                    <div class="qris">

                        <img src="<?= base_url() ?>assets/img/qris/Rek_Asmaf.jpg" class=" thumbnail rounded " style="width:200px !important; height: 200px !important;">

                    </div>

                </div>

            </div>

            <div class="carousel-item" data-bs-interval="10000">

                <div class="background">

                    <img src="<?= base_url() ?>assets/img/bg_ramadhan.jpg" class="d-block w-100" alt="...">

                </div>

                <div class="position-absolute top-0 start-50 translate-middle-x mt-3 m-auto text-center">

                    <h3 style="color: #fff;">DONOR DARAH</h3>                    
                    <div class="d-flex">
                        <p class="ps-3">Hari/Tanggal : Ahad, 2 April 2023</p>
                        <p class="ps-3">Tempat : Masjid Al Furqon Pakuan Regency</p>
                    </div>
                </div>
                
                <swiper-container class="mySwiper ms-5 position-absolute top-50 start-50 translate-middle text-center" navigation="false" space-between="30" centered-slides="true" autoplay-delay="2500" autoplay-disable-on-interaction="false">
                    <swiper-slide>
                        <div class="container-fluid ">
                            <div class="row text-center">
                                <div class="col-4">
                                    <img src="<?= base_url() ?>assets/img/kegiatanmasjid/donor_darah.jpg" class=" thumbnail rounded" style="width: 350px !important; height: 350px !important;">
                                </div>
                                <div class="col-4">
                                    <img src="<?= base_url() ?>assets/img/kegiatanmasjid/donor_darah.jpg" class=" thumbnail rounded" style="width: 350px !important; height: 350px !important;">
                                </div>
                                <div class="col-4">
                                    <img src="<?= base_url() ?>assets/img/kegiatanmasjid/donor_darah.jpg" class=" thumbnail rounded" style="width: 350px !important; height: 350px !important;">
                                </div>
                            </div>
                        </div>
                    </swiper-slide>
                    <swiper-slide>
                        <img src="<?= base_url() ?>assets/img/kegiatanmasjid/santunan.jpg" class=" thumbnail rounded float-end ms-5">
                    </swiper-slide>
                </swiper-container>
                
            </div>

            <div class="carousel-item" data-bs-interval="10000">

                <div class="background">

                    <img src="<?= base_url() ?>assets/img/bg_ramadhan2.jpg" class="d-block w-100" alt="...">

                </div>

                <div class="container position-absolute top-0 start-50 translate-middle-x mt-5 m-auto text-center">

                    <h1 style="color: #fff;" class="mt-5">Jadwal Shalat</h1>

                    <h5 style="color: #fff;">Hari/Tanggal : Ahad, 2 April 2023</h5>
                    <h5 class="ps-3" style="color: #fff;">Lokasi : Masjid Al Furqon Pakuan Regency</h5>

                    <div class="container mt-5">
                    <div class="row text-center">
                        <div class="col-12">
                            <table class="table" style="color: white !important;">
                                <thead>
                                    <tr>
                                        <th scope="col">Subuh</th>
                                        <th scope="col">Dzuhur</th>
                                        <th scope="col">Ashar</th>
                                        <th scope="col">Magrib</th>
                                        <th scope="col">Isya</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr>
                                        <th scope="row">1</th>
                                        <td>Mark</td>
                                        <td>Otto</td>
                                        <td>@mdo</td>
                                        <td>@mdo</td>
                                    </tr>
                                    <tr>
                                        <th scope="row">2</th>
                                        <td>Jacob</td>
                                        <td>Thornton</td>
                                        <td>@fat</td>
                                        <td>@fat</td>
                                    </tr>
                                    
                                </tbody>
                            </table>
                        </div>
                    </div>
                </div>
                </div>
            </div>

            <div class="areamarq position-absolute top-100 start-50 translate-middle">
                <marquee direction="left" style="margin-left:10vw !important">REKENING AL FURQON: (INFAQ) Bank Syariah Indonesia No Rek: 2021 2026 13 a.n Masjid Al Furqon | (SANTUNAN ANAK YATIM) Bank Syariah Indonesia No Rek: 2021 2026 35 | (WAKAF) Bank Syariah Indonesia No Rek: 2021 2026 46 </marquee>
            </div>
            <div class="position-absolute bottom-0 start-0" style="margin-left:8vw !important">
                <img src="<?= base_url() ?>assets/img/kegiatanmasjid/rounded_logo.png" class="mb-2" alt="" width="100">
            </div>

        </div>

    </div>

        <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.2.3/dist/js/bootstrap.bundle.min.js" integrity="sha384-kenU1KFdBIe4zVF0s0G1M5b4hcpxyD9F7jL+jjXkk+Q2h455rYXK/7HAuoJl+0I4" crossorigin="anonymous"></script>

         <!-- Swiper JS -->
         <script src="https://cdn.jsdelivr.net/npm/swiper@9/swiper-element-bundle.min.js"></script>
         <script src="https://cdn.jsdelivr.net/npm/swiper@9/swiper-bundle.min.js"></script>

         <!-- Initialize Swiper -->
         <script>
            const progressCircle = document.querySelector(".autoplay-progress svg");
            const progressContent = document.querySelector(".autoplay-progress span");

            const swiperEl = document.querySelector("swiper-container");
            swiperEl.addEventListener("autoplaytimeleft", (e) => {
            const [swiper, time, progress] = e.detail;
            progressCircle.style.setProperty("--progress", 1 - progress);
            progressContent.textContent = `${Math.ceil(time / 1000)}s`;
            });
         </script>

    </body>

</html>